...
class Function:
    ...
    def __init__(self, ID, returntype, name, parameterlist):
        ...
        if not isVoidDatatype(returntype):
            ...
            #rt = PointerDatatype(returntype.signature, returntype.declaration("") + "*", "p_out")
            parameterlist = chain(((rt, returnValueName),), parameterlist)
        self.name = name
...
{0}\t/*TODO: parse answer, fill return type*/;
...
def getGenericIntroduction(version):
    typedefs = "".join((
...
def generateCode(file):
    ...
    cfile = "{0}{1}".format(getGenericIntroduction("0.1"), "".join(f.getDefinition("sendBuffer", "send", 0) for f in functionlist))
    hfile = "{0}{1}".format(getGenericIntroduction("0.1"), "".join(f.getDeclaration() for f in functionlist))
    return cfile, hfile
    
cfile, hfile = generateCode("mintest.h")
#cfile, hfile = generateCode("test.h")
print(cfile, hfile)
print(len(cfile) + len(hfile), len(cfile.split("\n")) + len(hfile.split("\n")))
